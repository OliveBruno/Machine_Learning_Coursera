...
def cofiCostFunc(params, Y, R, num_users, num_movies,
    ...
    - The returned gradient will be the concatenation of the raveled 
      ...
      Concretely, you can set idx = np.where(R[i, :] == 1)[0] to be a list of all the users that have rated movie  𝑖 .
      This will allow you to create the temporary matrices Theta_temp = Theta[idx, :] and Y_temp = Y[i, idx] that index into 
      Theta and Y to give you only the set of users which have rated the  𝑖𝑡ℎ  movie. This will allow you to write the
      derivatives as: 
    ...
    J += np.sum((((X).dot(Theta.T))[R == 1] - (Y[R==1]))**2)/2 + (lambda_/2)*(np.sum(Theta**2) + np.sum(X**2))
